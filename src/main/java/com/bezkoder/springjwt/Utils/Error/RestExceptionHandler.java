package com.bezkoder.springjwt.Utils.Error;
...
public class RestExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    protected ResponseEntity<Object> handleNoHandlerFoundException(
        ...
        ApiError apiError = new ApiError(BAD_REQUEST);
        apiError.setError(String.format("Could not find the %s method for URL %s", ex.getHttpMethod(), ex.getRequestURL()));
    ...
    protected ResponseEntity<Object> handleMethodArgumentTypeMismatch(AccessDeniedException ex,
        ...
        if ("Access is denied".equals(ex.getMessage())) {
            apiError.setError(ErrorCode.ERR_NOT_ROLE.getMessage());
        } else {
            apiError.setError(ex.getMessage());
        }
    ...
    protected ResponseEntity<Object> handleCustomException(CustomException ex, WebRequest request) {
        ...
        if (ex.getErrorCodes() != null && ex.getErrorCodes().size() > 0) {
            apiError.setError(ex.getErrorCodes().get(0).getMessage());
        }
        apiError.setPath(((ServletWebRequest) request).getRequest().getRequestURI());
        return buildResponseEntity(apiError);
    }
